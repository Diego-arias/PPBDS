...
**PPBDS.data** includes the data sets for this book. **rsample** includes functions for bootstrapping. **rstanarm**, **broom.mixed**, **modelsummary** and **tidybayes** make it easy to create and display Bayesian models.
...
However, a little mathematical notation will make our modeling assumptions clear, will bring some clarity and precision to our approach. In this case:
...
The first argument in the `stan_glm()` function is `data`, which in our case is the filtered `ch7_male` tibble we used in the bootstrap example. The only other mandatory argument is the formula that we want to build a model around. In this case, since we have no predictor variables, our equation will be `height ~ 1`. Details:
...
#abline(v = 167.5)
=======
* There is a direct connection between the mathematical form of the model created under Justice an the code we use to fit the model under Courage. `height ~ 1` is the code equivalent of $$ y_i =  \mu $$. 
...
The first line is telling us what model we used, in our case a `stan_glm()`. 
...
Instead of printing the whole model, we can just print out the parameters.
...
Although it is possible to have variables names like (Intercept), it is not recommended. Avoid weird names! When you are stuck with them, place them in backticks. Better, rename them, as we do above.
...
<!-- DK: Could not get this to work. It seems like these gt objects are already text, so there is no easy way to, for example, change the formatting but adding decimals.  Maybe we should use model_summary, which seems to work nicely. -->

<!-- DK: Maybe I don't need to show posterior_linpred at all? One less thing to worry about. -->
...
```
=======
<!-- A third way to display the model is to use `modelsummary()` from the **modelsummary** package. (Note that this will often require the **broom.mixed** package as well.) -->
>>>>>>> master
...
<!-- ```{r} -->
<!-- library(modelsummary) -->
<!-- modelsummary(fit_obj,  -->
<!--              statistic = 'conf.int',  -->
<!--              statistic_vertical = FALSE, -->
<!--              title = "Average Height of American Men in 2009") -->
<!-- ``` -->


<!-- Keep in mind, again, that we are just looking at the same model in a different way. The `fit_obj` has not changed. Instead, we are just exploring a third way to consider it. Much of this information is the same that we have seen before. At the same time, other information is omitted. For example, and by default, `modelsummary()` does not even bother to show any numbers associated with $\sigma$. It doubts that you, or your readers, will care. -->


<!-- DK: More details. And don't forget to compare this confidence interval to the bootstrapped one. -->
...
Recall that a "matrix" in R is a rectangular array of data, shaped like a data frame or tibble, but containing only one type of data, i.e., numeric. Large matrices also print out ugly. (There are other differences, none of which we care about here.) Example:
...
That is a simple enough command, but the resulting object `pp`, a matrix, is not easy to work with. Note how big it is!
...
are `r ncol(pp)` because the matrix provides a (potentially) different posterior prediction for each of the input data rows. (In this case, they are all the same because the model does not use any covariates. In more complex models, the columns in the `pp` matrix can be very different.) 
...
There is about a 75% chance that, when meeting 4 random men, the tallest with be at least 10 cm taller than the shortest.
